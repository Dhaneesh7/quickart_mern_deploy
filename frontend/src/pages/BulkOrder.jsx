...
// import { CartToStore } from "../../../backend/controllers/orderControllers";
...
const BulkOrderPage = () => {
  ...
  const { addOrder, loading ,CartToStore} = useOrderStore();
const {cart} =useCartStore();
  const handleSubmit = async (e) => {
    e.preventDefault();
      // Fetch product data if needed (e.g., price)
//       console.log("product", product);
//   const productRes = await fetch(`http://localhost:5000/api/products/id/${product}`);
...
//   }
  //     const orderData = {
  //   ...formData,
  //   productId: product,  // product comes from useParams as a string product id
  // };
   const orderData = {
    name: formData.name,
  ...
  items: [
   ...
   // This is critical for Stripe calculation
  };
    ...
    const success = await CartToStore(orderData);
    if (success) {
      setFormData({
        name: "",
        email: "",
...
console.log("checkout session response:", data.sessionId);
